...
var SatVel  = 7.8E+04 // Orbital velocity in m/s
...
// Generate hyperbola points as two separate branches sorted by latitude
function generateHyperbolaBranches(centerLat, centerLon, X_val, Y_val, aDeg, rDeg, numPoints = 100) {
    const leftBranch = [];
    const rightBranch = [];
...
const iridiumSatellites = iridiumTLEs.map(sat => {
    ...
    return {name:sat.name, satrec, marker, coverageCircle, leftHyper, rightHyper};
});
