...
const rEarthKm = 6378.14; // Equitorial radius of Earth in km
const elevationAngleRad = dopplerAngleRad; // minimum elevation

const satelliteIcon = L.icon({
    iconUrl: 'iridium_ico.png',
    iconSize: [48, 48],
    iconAnchor: [16, 16],
    popupAnchor: [0, -16]
});

// compute angle of rotation from vector
const angle = 86.4*Math.PI/180; //Math.atan2(vector[1], vector[0]); // radians
console.log(angle);
...
function sortPointsAlongAngle(points, angleRad) {

    const cosA = Math.cos(angleRad);
    const sinA = Math.sin(angleRad);

    return points.sort((a, b) => {
        // Local coordinates relative to rotation axis
        const projA = a[0] * cosA + a[1] * sinA;
        const projB = b[0] * cosA + b[1] * sinA;
        return projA - projB;
    });
}
...
function rotatePoint(x, y, theta) {
    ...
    return [xr/1.11E+05, yr/1.11E+05];
}

// Generate hyperbola points (doppler arcs) as two separate branches sorted by latitude
function generateHyperbolaBranches(centerLat, centerLon, X_val, Y_val, aDeg, rDeg, vector, numPoints=1000) {
    var leftBranch = [];
    var rightBranch = [];

    // Left branch
    for (let x = -X_val; x <= -0.2; x += 2 / numPoints) {
        let val = (x*x)/(aDeg*aDeg) - 1;
        if (val < 0) continue;
        let y = Math.sqrt(val) * aDeg;
        if (x**2 < rDeg**2 + aDeg**2/2) {
            let [xr, yr] = rotatePoint(x*1.11E+05, y*1.11E+05, angle);
            leftBranch.push([centerLat + yr, centerLon + xr]);
            [xr, yr] = rotatePoint(x*1.11E+05, -y*1.11E+05, angle);
            leftBranch.push([centerLat + yr, centerLon + xr]);
        }
    }
    ...
    for (let x = 0.2; x <= X_val; x += 2 / numPoints) {
        ...
        if (x**2 < (rDeg**2 + aDeg**2)/2) {
            let [xr, yr] = rotatePoint(x*1.11E+05, y*1.11E+05, angle);
            rightBranch.push([centerLat + yr, centerLon + xr]);
            [xr, yr] = rotatePoint(x*1.11E+05, -y*1.11E+05, angle);
            rightBranch.push([centerLat + yr, centerLon + xr]);
        }
    }

    // Sort by latitude if needed
    leftBranch = sortPointsAlongAngle(leftBranch, angle);

    // Same for rightBranch
    rightBranch = sortPointsAlongAngle(rightBranch, angle);
    rightBranch.reverse();
...
function circleArcPoints(lat, lon, radiusDeg, lim, numPoints = 50) {
    ...
    for (let x = -lim; x <= lim; x += 2 / numPoints) {
        const pos_y = (180/Math.PI) * Math.acos(Math.cos(radiusDeg * Math.PI/180) / Math.cos(x * Math.PI/180));
        const neg_y = -(180/Math.PI) * Math.acos(Math.cos(radiusDeg * Math.PI/180) / Math.cos(x * Math.PI/180));
        let [pos_xc, pos_yc] = rotatePoint(x*1.11E+05, pos_y*1.11E+05, angle);
        let [neg_xc, neg_yc] = rotatePoint(x*1.11E+05, neg_y*1.11E+05, angle);
        upperArc.push([pos_yc + lat, pos_xc + lon]);
        lowerArc.push([neg_yc + lat, neg_xc + lon]);
    }
...
const iridiumSatellites = iridiumTLEs.map(sat => {
    ...
    marker.on('click', ()=>{
        ...
        coverageCircle.setStyle({opacity:0.5, fillOpacity:0.2});
        ...
        [leftBranch, rightBranch] = generateHyperbolaBranches(lat, lon, X_val, Y_val, hyp_param, radiusDeg, velocity);
        leftHyper.setLatLngs(leftBranch).setStyle({opacity:0});
        rightHyper.setLatLngs(rightBranch).setStyle({opacity:0});
        ...
        Polygon.setLatLngs(polygonCoords).setStyle({opacity:0, fillOpacity: 0.3});
...
function updatePositions(){
    ...
    iridiumSatellites.forEach(sat=>{
        ...
        if(activeCoverageCircle === sat.coverageCircle){
            ...
            sat.Polygon.setLatLngs(polygonCoords).setStyle({opacity:0, fillOpacity: 0.3});
